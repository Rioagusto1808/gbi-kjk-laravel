...
<div>
    ...
    <x-text-input id="judul" type="text" name="judul" value="{{ old('judul', $berita->judul) }}" class="w-full mt-1"
        required />
...
<div>
    ...
    <textarea id="isi" name="isi" rows="5" class="w-full mt-1 border-gray-300 rounded-md">{{ old('isi', $berita->isi) }}</textarea>
...
<div>
    ...
    <x-text-input id="published_at" type="datetime-local" name="published_at"
        value="{{ old('published_at', $berita->published_at?->format('Y-m-d\TH:i')) }}" class="w-full mt-1" />
