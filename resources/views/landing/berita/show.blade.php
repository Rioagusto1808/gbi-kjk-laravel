        ...
        <div class="lg:col-span-2">
            <article class="bg-white shadow rounded-lg p-6">
                ...
                @if ($berita->files->count())
                    <img src="{{ Storage::url($berita->files->first()->path) }}" 
                         alt="{{ $berita->judul }}" 
        ...
        <div>
            <div class="bg-white shadow rounded-lg p-4">
                ...
                @foreach ($beritaLain as $b)
                    <div class="flex items-start gap-3 mb-4 pb-4 border-b border-gray-200 last:border-0 last:mb-0 last:pb-0">
                        {{-- Thumbnail --}}
                        @if ($b->files->count())
                            <img src="{{ Storage::url($b->files->first()->path) }}" 
                                 alt="{{ $b->judul }}" class="w-20 h-16 object-cover rounded">
                        @else
                            <img src="{{ asset('images/no-image.jpg') }}" 
                                 alt="No Image" class="w-20 h-16 object-cover rounded">
                        @endif
